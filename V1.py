...
async def dispatch(body: DispatchRequest) -> DispatchResponse:
    ...
    status = "dry_run" if body.dry_run else ("sent" if sent else "not_configured")
    logger.info("Dispatch complete", slot=slot, sent=sent)
    return DispatchResponse(status=status, slot=slot, message=message, sent=sent)
